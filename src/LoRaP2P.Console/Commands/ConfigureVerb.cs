...
internal sealed class ConfigureVerb : IConsoleVerb
{
    ...
    {
        RadioConfiguration updated = Setting.ToLowerInvariant() switch
        {
            "frequency" => context.Configuration with
            {
                FrequencyHertz = ParseDouble(Value, "frequency") * 1_000_000,
            },
            "sf" => context.Configuration with
            {
                SpreadingFactor = (SpreadingFactor)ParseInteger(Value, "spreading factor"),
            },
            "bandwidth" => context.Configuration with
            {
                Bandwidth = (SignalBandwidth)(ParseInteger(Value, "bandwidth") * 1_000),
            },
            "power" => context.Configuration with
            {
                OutputPowerDbm = ParseInteger(Value, "power"),
            },
        ...
        context.Output.WriteLine("Radio configuration applied.");
